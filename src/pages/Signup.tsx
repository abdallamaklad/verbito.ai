...
export default function Signup() {
  ...
  const validate = () => {
    ...
    if (!name.trim()) newErrors.name = tt.nameRequired;
    ...
    if (!agreed) newErrors.agreed = tt.mustAgree;
    setErrors(newErrors);
    ...
    <div className="min-h-[100dvh] flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-600 to-indigo-700 items-center justify-center relative overflow-hidden">
        <div className="relative z-10 text-white text-center px-12">
          <h2 className="font-heading font-bold text-3xl mb-4">{tt.startJourneyTitle}</h2>
          <p className="text-violet-100 mb-8">{tt.startJourneyText}</p>
          <div className="flex items-center justify-center gap-8 text-sm text-violet-200">
            <div className="text-center"><p className="text-2xl font-bold text-white">12,400+</p><p>{tt.statPrompts}</p></div>
            <div className="text-center"><p className="text-2xl font-bold text-white">4.9/5</p><p>{tt.statRating}</p></div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-white dark:bg-gray-950">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{tt.getStartedFree}</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={tt.placeholderPasswordMin}
            ...
            <div>
              <label className="flex items-start gap-2 text-sm text-gray-600 dark:text-gray-400 cursor-pointer">
                ...
                />
                <span>{tt.agreeTerms} <Link to="/terms" className="text-violet-600 hover:underline">{tt.termsLink}</Link> and <Link to="/privacy" className="text-violet-600 hover:underline">{tt.privacyLink}</Link></span>
              </label>
